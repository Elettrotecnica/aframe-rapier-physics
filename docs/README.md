...
| scaleAutoUpdate     | —                                                               | `false`                           | Update the collision shape when the object's scale changes. Rapier does not support scaling the collision shape, so the entire collider must be recomputed. Use with caution. The shape can also be recomputed on demand without this parameter by using the updateShape method of this component. |
